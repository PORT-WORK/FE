...
import { Camera, Plus, X, Github, BookOpen, Figma, ExternalLink, Edit3 } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const removeSkill = (skill: string) => setUser(prev => ({ ...prev, skills: prev.skills.filter(item => item !== skill) }));
    ...
    <div className="px-8 py-8 overflow-y-auto" style={{ background: '#050505', minHeight: '100%' }}>
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setEditing(prev => !prev)} className="flex items-center gap-2 px-4 py-2 text-sm rounded-xl transition-all" style={{ background: editing ? 'rgba(124,58,237,0.12)' : 'rgba(255,255,255,0.04)', border: `1px solid ${editing ? 'rgba(124,58,237,0.25)' : 'rgba(255,255,255,0.07)'}`, color: editing ? '#a78bfa' : '#a1a1aa' }}>
            <Edit3 size={13} />{editing ? 'Save' : 'Edit'}
          </button>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <div className="absolute -bottom-6 left-6">
            <div className="relative w-16 h-16 rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg,#7c3aed,#2563eb)', boxShadow: '0 0 0 3px #050505' }}>
              <div className="w-full h-full flex items-center justify-center text-xl font-black text-white">M</div>
          ...
          <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                ['Career', user.career, 'career'],
                ['Email', user.email, 'email'],
              ].map(([label, value, key]) => (
                <div key={key as string}>
                  <p className="text-[10px] text-zinc-600 mb-1.5 uppercase tracking-wider">{label}</p>
                  {editing ? (
                    <input value={value as string} onChange={e => setUser(prev => ({ ...prev, [key as string]: e.target.value }))} className="w-full px-3 py-2 text-sm text-white rounded-xl focus:outline-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
                  ) : (
              ...
              <div className="col-span-2">
                ...
                {editing ? (
                  <textarea value={user.bio} onChange={e => setUser(prev => ({ ...prev, bio: e.target.value }))} rows={3} className="w-full px-3 py-2 text-sm text-white rounded-xl focus:outline-none resize-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
                ) : (
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={link.key} className="flex items-center gap-3">
                  ...
                  {editing ? (
                    <input value={(user.links as any)[link.key] || ''} onChange={e => setUser(prev => ({ ...prev, links: { ...prev.links, [link.key]: e.target.value } }))} className="flex-1 px-3 py-1.5 text-xs text-zinc-300 rounded-xl focus:outline-none" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }} />
                  ) : (
                    <span className="flex-1 text-xs text-zinc-500">{(user.links as any)[link.key] || '—'}</span>
                  )}
                </div>
              ))}
            </div>
