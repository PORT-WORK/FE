    ...
    <div className="px-8 py-8 overflow-y-auto" style={{ background: '#050505', minHeight: '100%' }}>
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-white">{ko ? '프로필' : 'Profile'}</h2>
        ...
        <div className="mt-10 space-y-5">
          <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-4">{ko ? '기본 정보' : 'Basic info'}</h3>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={key as string}>
                  <p className="text-[10px] text-zinc-600 mb-1.5 uppercase tracking-wider">{ko ? (label === 'Career years' ? '경력' : label === 'Location' ? '위치' : label === 'Email' ? '이메일' : '이름') : label}</p>
                  {editing ? (
                    <input
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {editing && (
                <div className="flex items-center gap-1.5">
                  <input value={newSkill} onChange={e => setNewSkill(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSkill()} placeholder={ko ? '스킬 추가...' : 'Add skill...'} className="px-3 py-1.5 text-xs text-white rounded-xl focus:outline-none w-28" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }} />
          ...
          <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-4">{ko ? '링크' : 'Links'}</h3>
