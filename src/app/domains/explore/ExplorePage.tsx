...
const SORT_OPTIONS = ['Latest', 'Popular', 'Most Viewed'];
      ...
      <div className="max-w-5xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-5">
          <div className="flex-1 relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('explore_search')} className="w-full pl-9 pr-9 py-2.5 text-sm text-zinc-300 placeholder-zinc-700 rounded-xl focus:outline-none" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }} />
            {search && <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-zinc-300"><X size={13} /></button>}
          </div>
          <button onClick={() => setFollowOnly(prev => !prev)} className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all flex-shrink-0" style={{ background: followOnly ? 'rgba(124,58,237,0.12)' : 'rgba(255,255,255,0.04)', border: `1px solid ${followOnly ? 'rgba(124,58,237,0.25)' : 'rgba(255,255,255,0.07)'}`, color: followOnly ? '#a78bfa' : '#71717a' }}>
            <Users size={12} />Following only
          </button>
          <div className="relative flex-shrink-0" onClick={e => e.stopPropagation()}>
            <button onClick={() => setSortOpen(prev => !prev)} className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs text-zinc-400 transition-all" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div className="flex items-center gap-1.5 mb-3 flex-wrap">
          {ROLES.map(role => (
            <button key={role} onClick={() => setActiveRole(role)} className="px-3 py-1.5 rounded-xl text-xs font-medium transition-all" style={{ background: activeRole === role ? 'rgba(124,58,237,0.12)' : 'rgba(255,255,255,0.03)', border: `1px solid ${activeRole === role ? 'rgba(124,58,237,0.25)' : 'rgba(255,255,255,0.06)'}`, color: activeRole === role ? '#a78bfa' : '#71717a' }}>
              {role}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5 mb-4 flex-wrap">
          {STACKS.map(stack => (
            <button key={stack} onClick={() => setActiveStack(activeStack === stack ? '' : stack)} className="px-3 py-1.5 rounded-xl text-xs transition-all" style={{ background: activeStack === stack ? 'rgba(37,99,235,0.12)' : 'rgba(255,255,255,0.02)', border: `1px solid ${activeStack === stack ? 'rgba(37,99,235,0.25)' : 'rgba(255,255,255,0.05)'}`, color: activeStack === stack ? '#60a5fa' : '#52525b' }}>
              {stack}
            </button>
          ))}
        </div>

        {!loading && <p className="text-xs text-zinc-600">{sorted.length} {t('explore_count')}</p>}
      </div>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-3 flex flex-col items-center justify-center py-20">
            ...
            <p className="text-xs text-zinc-700 mb-5">Try different keywords or reset filters.</p>
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {(user.skills || []).slice(0, 3).map((skill: string) => (
                    <span key={skill} className="px-2 py-0.5 text-[10px] text-zinc-500 rounded-md" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>{skill}</span>
                  ))}
                ...
                <div className="flex items-center justify-between pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={e => { e.stopPropagation(); setLiked(prev => { const next = new Set(prev); next.has(user.id) ? next.delete(user.id) : next.add(user.id); return next; }); }} className={`flex items-center gap-1 text-xs transition-all hover:scale-110 ${liked.has(user.id) ? 'text-red-400' : 'text-zinc-600 hover:text-zinc-300'}`}>
